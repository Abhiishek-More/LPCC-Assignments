...
public class Week2
{
	...
	{
		FileInputStream fis = new FileInputStream("D:\\Study\\3-TYBT\\LPCC LAB\\Assignment-2\\Week 2\\mcode.txt");
		...
		List<String> macroStore = new ArrayList<String>();
...
//	        MDT
			if(macro == true && arr[0].equals("MACRO") == false)
			{
				...
				{
					if(macroStore.contains(arr[i]))
					{
						int tn = macroStore.indexOf(arr[i]);
						String ts = Integer.toString(tn);
						codef = codef.replace(arr[i], "#" + tn);
					}
				}
				MDT.add(codef);
				mntc++;
...
//			INTERMEDIATE CODE GENERATION
		...
		System.out.println("MDT table is as follows:\n");
